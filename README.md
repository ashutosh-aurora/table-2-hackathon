...
We use the ecosystem's own words: reusable build know-how is **skills** (no new "craft" term), and
shared vocabulary is the **ubiquitous language**, realised as a glossary.
...
- `context/` — business, product-area (both consumed), team (authored on the day).
...
The strategy deck, the 21 themes (separate track), per-stage timeboxes (set by the dry-run), and the
